...
class ApunteModelTest(TestCase):
    """
    Suite de pruebas para el modelo Apunte.
    Valida la creación, validaciones, puntuación y eliminación de apuntes.
    """

    def setUp(self):
        """Configura los datos necesarios para las pruebas del modelo Apunte."""
    ...
    def test_apunte_creation(self):
        """Prueba la creación básica de un apunte y sus propiedades derivadas."""
    ...
    def test_puntuacion_logic(self):
        """
        Prueba el sistema de puntuación de apuntes.
        Verifica que se calculen correctamente el promedio, total y puntuación por usuario.
        """
        file = SimpleUploadedFile(
            "test_file.txt", b"content", content_type="text/plain"
        )
    ...
    def test_invalid_file_extension(self):
        ...
        Verifica la validación de seguridad contra archivos ejecutables maliciosos.
        """
        from django.core.exceptions import ValidationError

        file = SimpleUploadedFile(
            "virus.exe", b"malware", content_type="application/x-msdownload"
        )
    ...
    def test_file_size_limit(self):
        ...
        Valida el límite de tamaño de archivo implementado por seguridad y uso de almacenamiento.
        """
        from django.core.exceptions import ValidationError
        ...
        file = SimpleUploadedFile(
            "large.pdf", b"dummy content", content_type="application/pdf"
        )
        ...
        with self.assertRaisesRegex(
            ValidationError, "El archivo no puede superar 10MB"
        ):
            apunte.full_clean()
    ...
    def test_file_cleanup_on_delete(self):
        ...
        Verifica que la señal post_delete elimine los archivos huérfanos del sistema de archivos.
        """
        import os

        file_content = b"content to be deleted"
        file = SimpleUploadedFile(
            "delete_me.txt", file_content, content_type="text/plain"
        )
...
class ApunteViewTest(TestCase):
    """
    Suite de pruebas para las vistas de gestión de apuntes.
    Prueba funcionalidades de subida, puntuación, eliminación y permisos.
    """

    def setUp(self):
        """Configura el entorno de pruebas con usuarios, materias y apuntes de ejemplo."""
        self.client = Client()
        self.user = User.objects.create_user(username="author", password="password")
        self.user_voter = User.objects.create_user(
        ...
        self.file = SimpleUploadedFile(
            "test.pdf", b"pdf_content", content_type="application/pdf"
        )
    ...
    def test_subir_apunte_view(self):
        """
        Prueba la funcionalidad de subida de apuntes.
        Verifica que un usuario autenticado pueda subir un apunte correctamente
        y que se genere el mensaje de éxito correspondiente.
        """
    ...
    def test_subir_apunte_invalid_data(self):
        """
        Prueba el manejo de datos inválidos al subir un apunte.
        Verifica que se muestren los errores apropiados cuando faltan campos requeridos
        y que no se cree el apunte en la base de datos.
        """
        self.client.login(username="author", password="password")
        url = reverse("gestion_apuntes:subir_apunte", args=[self.materia.id])
    ...
    def test_puntuar_apunte_view(self):
        """
        Prueba la funcionalidad de puntuación de apuntes vía AJAX.
        Verifica que se pueda crear y actualizar una puntuación correctamente,
        y que se devuelva el promedio actualizado en formato JSON.
        """
    ...
    def test_eliminar_apunte_view_permission(self):
        """
        Prueba los permisos de eliminación de apuntes.
        Verifica que solo el autor del apunte pueda eliminarlo, rechazando
        intentos de otros usuarios y mostrando mensajes de error apropiados.
        """
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            Apunte.objects.filter(id=self.apunte.id).exists()
        )  # Should still exist
